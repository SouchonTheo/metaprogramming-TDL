...
public class EnumerationType implements Type, Declaration {
	...
	public boolean equalsTo(Type _other) {
		...
		if (otherType instanceof EnumerationType) {
			...
			if (other != null && other.getName().equals(this.getName()) && other.labels.size() == this.labels.size()) {
	...
	public boolean compatibleWith(Type _other) {
		...
		if (otherType instanceof EnumerationType) {
			EnumerationType other = (EnumerationType) otherType;
			if (other != null && other.labels.size() == this.labels.size()) {
				for (int i = 0 ; i < this.labels.size(); i++) {
					if (!this.labels.get(i).getType().compatibleWith(other.labels.get(i).getType())) {
						return false;
					}
				}
				return true;
