...
public abstract class AbstractField implements Expression {
	...
	public Type getType() {
		...
		Type otherType = _type;
		if (_type instanceof NamedType) {
			otherType = ((NamedType) _type).getType();
		}
		if (otherType instanceof RecordType) {
			recordType = (RecordType) otherType;
		} else {
			System.out.println("Type incompatible avec RecordType");
			return AtomicType.ErrorType;
		}
		if (recordType.contains(this.name)) {
			this.field = recordType.get(this.name);
		} else {
			System.out.println("Struct inconnue");
			return AtomicType.ErrorType;
		}
