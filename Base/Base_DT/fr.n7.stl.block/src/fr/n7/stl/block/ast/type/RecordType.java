...
public class RecordType implements Type, Declaration, Scope<FieldDeclaration> {
	...
	public boolean equalsTo(Type _other) {
		...
		if (otherType instanceof RecordType) {
			RecordType other = (RecordType) otherType;
			if (other != null && other.getName().equals(this.getName()) && other.getFields().size() == this.getFields().size()) {
				for (int i = 0 ; i < this.fields.size(); i++) {
					if (!this.getFields().get(i).equals(other.getFields().get(i))) {
						return false;
					}
				}
				return true;
	...
	public boolean compatibleWith(Type _other) {
		Type otherType = _other;
		if (otherType instanceof NamedType) {
			System.out.println("dans if NamedType\n\n");
			otherType = ((NamedType) _other).getType();
		}
		if (otherType instanceof RecordType) {
			System.out.println("dans if recordTpe\n\n");
			RecordType other = (RecordType) otherType;
			if (other != null && other.fields.size() == this.fields.size()) {
				for (int i = 0 ; i < this.fields.size(); i++) {
					if (!this.fields.get(i).getType().compatibleWith(other.fields.get(i).getType())) {
						return false;
					}
				}
				return true;
			...
			System.out.println("this type " + this.getType());
			System.out.println("other type " + _other);
			boolean b = _other instanceof RecordType;
			System.out.println("cond" + b);
			Logger.error("Type invalide.");
		}
		return false;
	}
	...
	public Type merge(Type _other) {
		...
		if (otherType instanceof RecordType) {
			RecordType otherRecord = (RecordType) otherType;
			List<FieldDeclaration> list = new ArrayList<FieldDeclaration>();
			for (int i = 0; i < fields.size(); i++) {
				Type t = this.fields.get(i).getType().merge(otherRecord.fields.get(i).getType());
				list.add(new FieldDeclaration(this.fields.get(i).getName(), t));
			}
