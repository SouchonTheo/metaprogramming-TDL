...
public class First implements Expression {
	...
	@Override
	public Type getType() {
		Type paramT = this.target.getType();
		/*
		 * if (paramT instanceof NamedType) {
		 * 	  return ((NamedType) paramT).getType();
		 * } else 
		 */
		if (paramT instanceof CoupleType) {
			return ((CoupleType) paramT).getFirst();
		} else {
			System.out.println("First pas sur une paire.");
			return null;
		}
	}
	...
	public Fragment getCode(TAMFactory _factory) {
		...
		frag.add(_factory.createPop(0, c.getSecond().length()));
		return frag;
	}
